...
check_no_conflict_markers() {
    ...
    if rg -n "$patterns" . \
        ...
        --glob '!venv/**' \
        --glob '!*.min.js' \
        --glob '!*.lock' >/tmp/pusha_conflict_markers.txt 2>/dev/null; then
        cat /tmp/pusha_conflict_markers.txt
        rm -f /tmp/pusha_conflict_markers.txt
        return 1
    fi
    rm -f /tmp/pusha_conflict_markers.txt
    return 0
...
smoke_launch_voice() {
    ...
    local smoke_log="/tmp/pusha_pre_push_smoke.log"
...
print_header "Pusha Talk Pre-Push Checks"
echo "Mode: $MODE"

run_check "No merge conflict markers" check_no_conflict_markers
run_check "Python syntax compile (pusha_talk.py)" \
    env PYTHONPYCACHEPREFIX=/tmp/pycache python3 -m py_compile pusha_talk.py
